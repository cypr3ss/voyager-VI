    ...
    */
	...
	switch(typeof (parseInt(mbits[0]) || mbits[0])){ /*deals with lack of -r*/
		case 'number':
			...
			if(mbits[1] === "@" && mbits[2]) {
				...
				loger[1] = mbits.join(" ").substring(mbits.join(" ").indexOf("\""), mbits.join(" ").indexOf("\"", mbits.join(" ").indexOf("\"") + 1) + 1).substring(1, mbits.join(" ").substring(mbits.join(" ").indexOf("\""), mbits.join(" ").indexOf("\"", mbits.join(" ").indexOf("\"") + 1) + 1).length - 1);
			}
			loger[2] = false;
		break;
		case 'string':
    ...
    if (loger[2]) {
	    global.vVIinters.push(setInterval(()=>{
		    channel.send(loger[1]);
	    }, loger[2]*1000));
    } else {
        global.vVIinters.push(setTimeout(()=>{
		    channel.send(loger[1]);
	    }, loger[2]*1000));
    }
};
